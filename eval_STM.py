...
### My libs
from STM_DS import STM_DAVIS_DS
from STM import STM
...
def get_arguments():
    ...
    parser.add_argument("--mem-every", type=int, default=15)
    ...
    parser.add_argument("--user", help="Use user dataset", action="store_true")
...
if args.user:
    palette = Image.open(os.path.join(DATA_ROOT, 'mask_palette.png')).getpalette()
else:
...
def Run_video(Fs, Ms, num_frames, num_objects, seq_name, Mem_every=None, Mem_number=None):
    ...
    test_path = os.path.join('./output', code_name, seq_name)
    if not os.path.exists(test_path):
        os.makedirs(test_path)

    pred = np.argmax(pred_mask[0].cpu().numpy(), axis=0).astype(np.uint8)
    img_E = Image.fromarray(pred)
    img_E.putpalette(palette)
    img_E.save(os.path.join(test_path, '{:05d}.png'.format(0)))


    for t in tqdm.tqdm(range(1, num_frames)):
        # memorize
        with torch.no_grad():
            prev_key, prev_value = model(Fs[:,:,t-1], pred_mask, torch.tensor([num_objects]))
        ...
        pred_mask = F.softmax(logit, dim=1)
        pred = np.argmax(pred_mask[0].cpu().numpy(), axis=0).astype(np.uint8)
        img_E = Image.fromarray(pred)
        img_E.putpalette(palette)
        img_E.save(os.path.join(test_path, '{:05d}.png'.format(t)))


        # label = torch.argmax(Ms[:, :, t], dim=1).long().cuda()
...
if args.user:
    Testset = STM_USER_Test_DS(args.D, output_size=720)
else:
    Testset = STM_DAVIS_DS(DATA_ROOT, resolution='480p', imset='20{}/{}.txt'.format(YEAR,SET), single_object=(YEAR==16))
...
pth_path = 'STM_Parallel_weights.pth'
print('Loading weights:', pth_path)
model.load_state_dict(torch.load(pth_path))

if args.user:
    code_name = '{}_DAVIS_USER'.format(MODEL)
else:
...
for seq, V in enumerate(Testloader):
    ...
    print('[{}]: num_frames: {}, num_objects: {}'.format(seq_name, num_frames, num_objects.item()))
    # pred, Es = Run_video(Fs, Ms, num_frames, num_objects, Mem_every=args.mem_every, Mem_number=None)
    Run_video(Fs, Ms, num_frames, num_objects, seq_name, Mem_every=args.mem_every, Mem_number=None)
    ...
    if not os.path.exists(test_path):
        os.makedirs(test_path)
    for f in tqdm.tqdm(range(num_frames), desc='save'):
